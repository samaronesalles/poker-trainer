...
Testes de contrato (Node 18+, sem bundler): motor de melhor 5 e enumerador de upgrades em `tests/contract/motor.test.js`, FSM do HUD em `tests/contract/hud-session.test.js`, quiz em `tests/contract/quiz.test.js`, persistência em `tests/contract/storage.test.js`, áudio fail-open e o gerador em `tests/contract/baralho.test.js`. Módulos de domínio: `js/motor.js` (`avaliarMelhor5`, `enumerarUpgrades`, `conjuntoOpcoesUpgrade`), `js/quiz.js` e `js/storage.js`. No flop e no turn do herói a certa é a melhor 5 real. Os upgrades do flop/turn usam a lista RN-020 real (não mais o stub Flush-verdadeiro nem o skip forçado do turn); lista vazia → skip; enumeração inválida aborta a mão. O river do herói e o showdown continuam stub até a feature 006. Sem Playwright.
