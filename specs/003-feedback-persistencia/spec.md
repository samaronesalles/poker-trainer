...
O treinando vê uma pergunta de seleção única no HUD (mão atual ou “quem ganhou o pote”). Clica numa opção: a resposta **submete na hora**, sem botão **Confirmar**. Se acertou, o HUD diz **“Você acertou”** e o estado visual da opção correta distingue o acerto do erro — texto **e** aparência, nunca só cor, nunca diálogo nativo do navegador. Se errou, o HUD diz **“Não é essa. Tente de novo.”**, **não** revela qual era a certa, a opção clicada permanece visível no mesmo lugar em estado eliminada (não clicável de novo nesta pergunta) e ele escolhe outra até acertar. A certa só aparece como certa quando ele a escolhe.
...
2. **Given** o clique na opção correta, **When** o feedback aparece, **Then** o HUD mostra exatamente **“Você acertou”**, no próprio painel da mesa (não no topo da página e não em diálogo nativo), e a opção correta tem estado visual de acerto distinguível do erro por **texto e** aparência (RN-034, CA-025).
...
4. **Given** um erro nesta pergunta, **When** o treinando olha as opções, **Then** a opção errada permanece visível no **mesmo lugar**, em estado eliminada (não clicável), e as demais opções ainda disponíveis permanecem na mesma ordem visual (RN-036).
...
6. **Given** uma opção já eliminada, **When** o treinando tenta ativá-la de novo (clique, Enter ou Espaço), **Then** a ação é ignorada e a pergunta não avança.
...
**Independent Test**: No flop, após o acerto da mão atual do stub, usar o **stub de múltipla seleção** desta feature (conjunto correto provisório): marcar, **Confirmar**, exercitar falso positivo e omissão, ver opções mortas/travadas, retry até o conjunto exibido estar correto; o turn permanece no skip `sem_upgrade` do casco até a feature 005.
...
2. **Given** o conjunto das opções **exibidas** está correto, **When** o treinando aciona **Confirmar**, **Then** o HUD mostra **“Você acertou”** (texto e estado visual) e a pergunta pode avançar (RN-034).
3. **Given** o treinando marca um upgrade verdadeiro **e** uma distratora, **When** aciona a **primeira Confirmar**, **Then** a distratora fica eliminada (visível, morta), o upgrade verdadeiro marcado trava (não desmarcar), o HUD pede de novo com **“Não é essa. Tente de novo.”** e **não** revela quais das restantes são obrigatórias (RN-035, RN-036).
...
6. **Given** o turn após o acerto da mão atual, **enquanto** o motor de upgrades da feature 005 não existir, **When** o HUD segue a cadência, **Then** permanece o skip `sem_upgrade` (**“Não há upgrade possível.”** + **Continuar**), **sem** múltipla seleção nessa street — os dois caminhos (Confirmar vs. skip) continuam observáveis.
7. **Given** `sem_upgrade`, **When** o treinando aciona **Continuar**, **Then** nenhum contador de `upgrade` muda e a próxima street pode abrir.

---

### User Story 3 - Guardar só a primeira tentativa, e sobreviver a fechar a aba (Priority: P1)

Cada pergunta expõe o treinando uma vez para a memória de desempenho: o **primeiro** clique (seleção única) ou a **primeira Confirmar** (múltipla seleção) atualiza os contadores. Errou e depois acertou na mesma pergunta: aquela exposição é **erro**, não acerto. Chutar “Flush” quando a certa é **Par** registra erro em **Par**, não em Flush. No upgrade, marcar uma distratora (falso positivo) conta **erro** daquela categoria; **não** marcar uma distratora **não** incrementa. Os totais ficam no dispositivo, na mesma origem: fechar e reabrir o app preserva, por exemplo, um acerto de primeira em **Flush** como mão atual. Não se grava nome, e-mail, apelido digitado, cartas da mão nem replay.
...
7. **Given** recarregar no meio da mão, **When** o HUD volta a `ociosa`, **Then** a mão em curso perde-se (casco), mas os contadores **já gravados** **não** são apagados por esse reset visual.
8. **Given** a persistência, **When** se inspeciona o que foi guardado, **Then** há só contadores de treino (`mao_atual`, `upgrade`, `vencedor_pote`); **não** há nome, e-mail, apelido digitado, foto, trajetória, replay de cartas nem identificador pessoal (RN-039, RN-040).
...
3. **Given** falha de persistência, **When** o treinando olha a UI, **Then** **não** há pedido de CPF, e-mail, nome real, apelido digitado ou outro identificador para “salvar o progresso”.

---

### Edge Cases

- Clique ou ativação de teclado em opção já eliminada: ignora; não avança; não altera contadores.
...
- Segunda **Confirmar** (e seguintes): não altera `upgrade`; só corrige o conjunto exibido.
- Skip `sem_upgrade`: nenhum contador `upgrade` muda (equivalente ao CA-017 quando a lista é vazia).
- Recarregar no meio da mão: mão aborta, HUD `ociosa`; contadores já persistidos permanecem; MUST NOT pedir dado pessoal para retomar.
...
- Dados corrompidos: descarta o bloco, zera contadores, mesa intacta.
...
- Preferência por reduzir movimento: o contrato de quiz (submissão, texto, opção morta, persistência) não depende de animação; o beat curto de acerto MAY ser imediato.
- Ordem visual: embaralha **ao apresentar** cada pergunta; MUST NOT reembaralhar após erro na mesma pergunta.
- Stub vs. feltro: até 004–006, a opção “certa” do stub MAY não coincidir com o board real; os contadores desta feature seguem a correta **do contrato da pergunta em vigor** (stub agora; motor depois). Para CA-023 ser testável antes da 004, o stub MUST permitir uma exposição de mão atual cuja correta seja **Flush**.
...
- Teclado: **Confirmar**, **Continuar** e as opções visíveis permanecem alcançáveis por Tab e ativáveis com Enter/Espaço (continuidade do casco); o mouse continua o caminho principal.
...
- **FR-003**: Acerto MUST ser explícito no HUD com o texto **“Você acertou”**. Erro MUST ser explícito no HUD com o texto **“Não é essa. Tente de novo.”**. Feedback MUST ocorrer no HUD da mesa, MUST NOT usar `alert()` nem diálogo nativo equivalente, e MUST distinguir acerto de erro por **texto e** estado visual (não só por cor) (RN-034, RN-035, CA-025).
- **FR-004**: Após erro, a opção incorreta MUST permanecer visível no mesmo lugar, em estado eliminada, e MUST NOT ser ativável de novo naquela pergunta. Ativação MUST ser ignorada (RN-036).
...
- **FR-008**: Só a **primeira tentativa** altera contadores: primeiro clique em seleção única; primeira **Confirmar** em múltipla seleção. Tentativas seguintes MUST NOT alterar contadores (RN-037, RN-019, RN-026).
...
- **FR-011**: Depois da primeira **Confirmar** de upgrades: distratoras marcadas MUST desabilitar; upgrades já marcados corretamente MUST travar (não desmarcar); upgrades verdadeiros ainda não marcados MUST permanecer selecionáveis. Novas **Confirmar** até o conjunto exibido estar perfeito MUST NOT alterar os contadores da primeira vez (RN-025, RN-026).
- **FR-012**: O modelo persistido MUST ter exatamente: `mao_atual` (por cada rótulo canônico de categoria — acertos 1ª, erros 1ª, exposições); `upgrade` (idem); `vencedor_pote` (um único grupo acertos/erros/exposições). No MVP, exposições MUST ser iguais a acertos + erros daquele bucket/categoria. MUST NOT contar “viu como distratora e não marcou” como exposição (RN-037).
...
- **FR-014**: Recarregar no meio da mão MUST abortar a mão (HUD `ociosa`) e MUST NOT apagar os contadores já persistidos.
- **FR-015**: MUST NOT persistir nome, e-mail, CPF, telefone, apelido digitado, foto, trajetória de ponteiro, replay de cartas, enunciados ou qualquer identificador pessoal. MUST NOT enviar mãos, respostas, contadores ou telemetria a servidor (RN-039, RN-040).
...
- **FR-024**: **Confirmar**, as opções visíveis e os CTAs já existentes do HUD MUST permanecer alcançáveis por Tab e ativáveis com Enter ou Espaço. MUST NOT exigir teclado para treinar.
...
- **Opção**: rótulo visível (categoria canônica ou texto de quem ganhou o pote). Estados: padrão, hover, foco de teclado, selecionada (só multi-select), correta (após acerto), eliminada/morta (após erro ou falso positivo). Eliminada permanece no lugar e não é ativável nesta pergunta.
- **Primeira tentativa**: o primeiro clique que submete (seleção única) ou a primeira **Confirmar** (múltipla seleção) daquela pergunta. É o único evento que altera contadores.
- **Evolução de treino**: memória no dispositivo, na origem do app. Três buckets: `mao_atual` (por categoria), `upgrade` (por categoria), `vencedor_pote` (grupo único). Em cada bucket: acertos de 1ª, erros de 1ª, exposições (= acertos + erros no MVP). Sem PII, sem replay de mão.
...
- **SC-001**: Em 100% dos acertos observáveis, o treinando vê **“Você acertou”** no HUD em menos de 1 segundo após a submissão, com estado visual de acerto, e 0 diálogos nativos de feedback (CA-025).
- **SC-002**: Em 100% dos erros observáveis, o treinando vê **“Não é essa. Tente de novo.”** no HUD, a opção errada permanece visível e inativa no mesmo lugar, e 0 revelações da certa antes do acerto (RN-035, RN-036).
...
- **Stub de múltipla seleção (escolha autônoma)**: para a US2 ser testável sem a feature 005, o **flop** após acerto da mão atual apresenta multi-select stub + **Confirmar**; o **turn** permanece `sem_upgrade`. 005 substitui o stub e passa a decidir skip vs. lista real nas duas streets. MUST NOT deixar esta feature bloqueada à espera da enumeração de information set.
...
- **Falha de storage (escolha autônoma)**: degradação **silenciosa** — sem linha extra no HUD (diferente da falha de embaralhar da 002, que tem copy própria). Treino segue; evolução pode não sobreviver ao fechar.
...
- **Privacidade**: apelidos fixos **Você**, **Adversário A**, **Adversário B**; avatares ilustrados. Recarregar aborta a mão e **não** apaga evolução. Coordenadas de ponteiro da 002 continuam só em memória da visita — esta feature MUST NOT passá-las à persistência.
...
- **Beat de acerto**: após “Você acertou”, um beat curto e a próxima pergunta/street — sem tela “Parabéns, fase 2” (PRD §8.1). O teto de animação de cartas continua o do casco; este beat é do HUD.
