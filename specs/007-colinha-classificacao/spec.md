...
- Q: Se as cinco cartas-exemplo fixas de uma linha coincidirem por acaso com faces que estão no feltro nesta mão, a implementação deve trocar o exemplo? → A: Não: coincidência de faces é permitida. “Não são as da mão” significa que os exemplos não vêm do baralho da rodada e não mudam; MUST NOT recalcular nem embaralhar o conjunto para evitar faces iguais às do feltro.
- Q: A colinha visível deve prender o teclado como um diálogo modal (armadilha de foco)? → A: Não: a colinha não é modal. MUST NOT prender o Tab dentro do overlay; o treinando MUST poder sair da colinha rumo à mesa e ao HUD. O quiz permanece operável com a colinha aberta.
- Q: Depois que o treinando oculta ou reabre a colinha só com o teclado, para onde vai o foco? → A: **Ocultar** move o foco para o botão **Colinha**; **Colinha** move o foco para **Ocultar**. Abrir ou recarregar a página MUST NOT colocar o foco na colinha.
- Q: Além do botão Ocultar, a colinha some ao pressionar Escape ou ao clicar no feltro? → A: Não: só o controle **Ocultar** oculta o painel. Escape e clique fora (feltro, cartas, HUD) MUST NOT dispensar a colinha.
- Q: Ao passar o mouse ou o foco de teclado por uma linha da lista, a linha pode ganhar destaque de seleção? → A: Não: as linhas MUST NOT ganhar hover, foco ou cursor de “clicável” que pareça opção de quiz. Sem chrome de seleção nas linhas.

### Session 2026-09-07 (2)

- Q: Na ordem de Tab da página, os controles da colinha vêm antes ou depois dos CTAs e opções do HUD? → A: Depois do HUD. Em `ociosa`, a primeira Tab alcança **Nova mão**, não **Ocultar**. A colinha é cromo; a mesa permanece o herói.
- Q: Os indicadores de sentido da lista devem mostrar exatamente as palavras Melhor e Pior? → A: Sim: exatamente **Melhor** no topo e **Pior** na base — sem “Best”, “Worst”, setas sem texto nem sinônimos.
- Q: Se as 10 linhas não couberem no canto sem cobrir comunitárias, o que a colinha faz? → A: No 1280×720 a 100% MUST caber sem cobrir. Fora disso, MAY encolher miniaturas ou rolar só dentro do canto; MUST NOT cobrir comunitárias, assentos nem HUD e MUST NOT empurrar o feltro.
...
3. **Given** a colinha visível no desktop, **When** o treinando lê o painel, **Then** há o texto visível **Melhor** no topo e **Pior** na base (rótulos exatos), a lista é vertical e compacta, e o visual é de clube (escuro, feltro/rail) — não um infográfico vermelho de site.
4. **Given** o viewport de referência 1280×720, **When** o treinando consulta as linhas, **Then** os rótulos e as cartas-exemplo permanecem legíveis a cerca de 70 cm do monitor e as 10 linhas cabem no canto **sem** cobrir comunitárias (RN-050).
5. **Given** uma janela larga mas mais baixa que o 1280×720, ou zoom acima de 100%, **When** as 10 linhas não cabem no canto, **Then** a colinha encolhe as miniaturas ou rola só dentro do overlay; comunitárias, assentos e HUD permanecem descobertos e o feltro **não** é empurrado.

---

### User Story 2 - Consultar as 10 categorias com exemplos fixos (Priority: P1)

O treinando lê a lista de cima para baixo. Cada linha tem o número de ordem (1 a 10), o rótulo canônico e cinco cartas-exemplo. As cartas são ilustrativas e **fixas**: não vêm do baralho da mão em curso e não mudam de uma mão para outra nem entre visitas. Cartas que não entram no conjunto principal daquela categoria (kicker da quadra, kickers de par, cartas de carta alta que não são a mais alta, etc.) ficam esmaecidas. Os nomes são exatamente os do quiz — sem “Sequência”, “Um par” ou “Straight flush real”.
...
**Independent Test**: Com a colinha visível, conferir as 10 linhas na ordem 1→10, os dez rótulos canônicos, cinco cartas-exemplo por linha no idioma visual da mesa, extras esmaecidas, e que os exemplos são ilustração fixa (não vêm do baralho da mão) e não mudam ao pedir **Nova mão** / **Próxima mão**.
...
5. **Given** uma mão em curso no feltro, **When** o treinando compara as cartas da colinha com o board e as hole cards, **Then** os exemplos da colinha **não** vêm do baralho daquela mão (são ilustração fixa, independente do deal); se alguma face coincidir por acaso com o feltro, o exemplo **permanece** o mesmo (RN-049).
...
2. **Given** a colinha visível no desktop, **When** o treinando pressiona Escape ou clica no feltro, nas cartas ou no HUD, **Then** o painel **permanece** visível; só **Ocultar** o dispensa.
3. **Given** a colinha ocultada, **When** o treinando aciona **Colinha**, **Then** o painel volta no mesmo canto com o título **Classificação de mãos** e as mesmas 10 linhas e cartas-exemplo (CA-029).
4. **Given** a colinha ocultada no desktop, **When** o treinando recarrega a página, **Then** a colinha está **visível** de novo; a evolução persistida (se houver) permanece inalterada (CA-029, RN-051).
5. **Given** a colinha ocultada no desktop, **When** se inspeciona o armazenamento da origem, **Then** a única chave de produto continua a da evolução (`mao_atual`, `upgrade`, `vencedor_pote`); **não** existe chave de preferência da colinha em armazenamento local, de sessão, cookie ou equivalente (CA-032, RN-051).
6. **Given** o painel visível ou o botão **Colinha**, **When** o treinando usa só o teclado, **Then** alcança **Ocultar** ou **Colinha** com Tab e ativa com Enter ou Espaço, com foco visível (RN-054).
7. **Given** o estado oculto, **When** o treinando compara o botão **Colinha** com os CTAs do HUD, **Then** **Colinha** é mínimo e **não** compete visualmente com **Nova mão** / **Próxima mão**.
8. **Given** o painel visível e o foco em **Ocultar**, **When** o treinando ativa **Ocultar** pelo teclado, **Then** o foco vai para o botão **Colinha** (não se perde nem salta para o HUD).
9. **Given** o painel oculto e o foco em **Colinha**, **When** o treinando ativa **Colinha** pelo teclado, **Then** o painel reabre e o foco vai para **Ocultar**.
10. **Given** o app acaba de abrir ou recarregar no desktop, **When** o treinando ainda não teclou Tab, **Then** o foco **não** está na colinha (nem em **Ocultar**).
11. **Given** o HUD em `ociosa` no desktop com a colinha visível, **When** o treinando pressiona Tab pela primeira vez, **Then** o foco vai para **Nova mão** (ou o CTA/opção visível do HUD), **não** para **Ocultar**.
...
6. **Given** a colinha visível e o HUD em `perguntando` ou `ociosa`, **When** o treinando percorre o Tab a partir de **Ocultar**, **Then** o foco **sai** da colinha e alcança o HUD (CTA ou opções); o overlay **não** prende o teclado.
7. **Given** a colinha visível, **When** o treinando passa o mouse ou tenta focar uma linha da lista, **Then** a linha **não** ganha destaque de seleção, hover de opção nem cursor de “clicável”; permanece consulta visual.
...
- Hover ou tentativa de foco numa linha: sem chrome de seleção, sem cursor de ponteiro; a linha não parece opção de quiz.
- Tab com a colinha visível: **não** há armadilha de foco; o ciclo continua na mesa/HUD. A colinha **não** é diálogo modal. Na ordem da página, **Ocultar**/**Colinha** vêm **depois** dos CTAs e opções do HUD; em `ociosa` a primeira Tab é **Nova mão**.
- Foco após toggle pelo teclado: **Ocultar** → **Colinha**; **Colinha** → **Ocultar**. Reload/abertura MUST NOT autofocar a colinha.
- Escape ou clique no feltro/cartas/HUD com a colinha visível: o painel **não** some; só **Ocultar** oculta.
...
- 10 linhas não cabem no canto (janela larga baixa ou zoom > 100%): encolher miniaturas ou rolar só no overlay; MUST NOT cobrir comunitárias/assentos/HUD nem empurrar o feltro. No 1280×720 a 100% MUST caber sem rolar.
- Primeira visita e visita seguinte (após reload): ambas abrem com a colinha **visível** no desktop; não há onboarding da colinha.
- Faces do exemplo iguais às do feltro nesta mão: permitido. MUST NOT trocar, embaralhar nem omitir a linha para “descoincidir”.
...
- Título ou controles em outro idioma / sinônimo de categoria: proibidos. Controles exatamente **Ocultar** e **Colinha**; título exatamente **Classificação de mãos**; indicadores de sentido exatamente **Melhor** (topo) e **Pior** (base).
...
- **FR-002**: Cada linha MUST mostrar o número de ordem (1 a 10), o rótulo canônico e **cinco** cartas-exemplo ilustrativas. Os exemplos MUST ser ilustração fixa, **não** extraídos do baralho da mão em curso. O exemplo de cada categoria MUST ser estável entre mãos e entre visitas (não embaralha a cada rodada). Coincidência de faces com o feltro é permitida: MUST NOT recalcular o conjunto para evitá-la (RN-049).
...
- **FR-005**: No layout largo da mesa (largura acima de 900 px, o mesmo recorte da mesa imersiva), a colinha MUST iniciar **visível** no canto **superior direito**, em overlay que **não** empurra o feltro. MUST NOT cobrir comunitárias, hole cards, assentos nem o HUD. No viewport de referência **1280×720** (zoom 100%) as 10 linhas MUST caber no canto, e os rótulos e as cartas-exemplo MUST permanecer legíveis. Fora desse viewport, se as 10 linhas não couberem, a colinha MAY encolher as miniaturas ou rolar **somente** dentro do overlay — MUST NOT cobrir a mesa nem empurrar o feltro (RN-050, CA-028).
- **FR-006**: O título visível MUST ser exatamente **Classificação de mãos**. MUST haver os rótulos visíveis exatamente **Melhor** (topo) e **Pior** (base) — sem “Best”, “Worst”, setas sem texto nem sinônimos. A lista MUST ser vertical e compacta. O cromo MUST ser de clube (escuro, feltro/rail), não infográfico vermelho de site (RN-054).
- **FR-007**: O treinando MUST poder ocultar e reabrir a colinha na **mesma visita**. Oculta → só o controle discreto **Colinha** no mesmo canto. Visível → controle **Ocultar** no painel. Só **Ocultar** MUST ocultar o painel: Escape e clique fora (feltro, cartas, HUD) MUST NOT dispensá-lo. MUST NOT gravar essa preferência em armazenamento local, de sessão, cookie ou outra persistência. Recarregar a página no desktop MUST restaurar a colinha **visível** (RN-051, CA-029, CA-032).
- **FR-008**: A colinha MUST ser **estática** em relação ao treino: MUST NOT destacar a categoria da mesa, MUST NOT marcar a opção certa do quiz, MUST NOT submeter resposta. Clique ou tecla em uma linha da lista MUST NOT alterar o HUD, as opções, os contadores nem o andamento da mão. As linhas MUST NOT ganhar hover, foco de seleção nem cursor de “clicável” que as faça parecer opções de quiz (RN-052, CA-030, RN-G005).
- **FR-009**: Em viewport com largura ≤ 900 px, MUST NOT existir o painel de classificação nem o botão **Colinha** (RN-053, CA-031).
- **FR-010**: A colinha MUST NOT ser pergunta, MUST NOT entrar no HUD, MUST NOT incrementar evolução e MUST NOT persistir cartas, information set, snapshot, runout ou dump de resultado. Controles visíveis MUST estar em português brasileiro: **Ocultar** e **Colinha**, alcançáveis por Tab e ativáveis com Enter ou Espaço; o mouse permanece o caminho principal. Linhas da lista MUST NOT entrar no ciclo de Tab como se fossem opções de quiz. A colinha MUST NOT ser diálogo modal: MUST NOT prender o foco no overlay; Tab MUST poder sair rumo à mesa e ao HUD. Na ordem de Tab da página, **Ocultar** e **Colinha** MUST vir **depois** dos CTAs e opções visíveis do HUD (em `ociosa`, a primeira Tab MUST alcançar **Nova mão**). Ativar **Ocultar** pelo teclado MUST mover o foco para **Colinha**; ativar **Colinha** MUST mover o foco para **Ocultar**. Abrir ou recarregar a página MUST NOT autofocar a colinha (RN-054).
...
- **Colinha**: overlay compacto de consulta no canto superior direito do desktop. Atributos: título **Classificação de mãos**, visibilidade de visita (`visível` | `oculto`), dez linhas canônicas. Não é pergunta, não é HUD, não é relatório e **não** é diálogo modal.
...
- **SC-001**: Em 100% das aberturas no desktop de referência 1280×720 a 100% de zoom (primeira visita ou após reload), o treinando vê a colinha no canto superior direito com as 10 linhas canônicas e cinco cartas-exemplo cada, em menos de 3 segundos após a tela estar pronta, cabendo no canto sem cobrir comunitárias, assentos ou HUD (CA-028).
...
- **SC-003**: Em 100% das perguntas ativas, 0 linhas da colinha aparecem marcadas como “a certa”; 0 cliques ou teclas em linha alteram enunciado, opções ou contadores; 0 linhas ganham hover/foco de seleção que as faça parecer opção de quiz (CA-030).
...
- **SC-007**: 100% dos controles visíveis **Ocultar** e **Colinha** podem ser focados via Tab e ativados via Enter ou Espaço, sem mouse. Em 100% dos toggles pelo teclado, o foco permanece no controle que ficou visível (**Colinha** após ocultar, **Ocultar** após reabrir). Em 100% das aberturas/reloads, o foco inicial **não** está na colinha. Em 100% das aberturas em `ociosa` no desktop, a primeira Tab alcança **Nova mão**, não **Ocultar**.
- **SC-008**: Em 100% das linhas, o rótulo visível é um dos 10 canônicos na ordem 1→10; 0 sinônimos aparecem na colinha. Em 100% das aberturas com o painel visível, os indicadores de sentido são exatamente **Melhor** (topo) e **Pior** (base).
...
- **Exemplos de cartas**: esta spec não congela rank/naipe de cada linha. O implementador escolhe **um** conjunto ilustrativo estável por categoria que deixe a categoria inequívoca (royal ≠ straight flush; wheel suited é straight flush; straight não circular). O conjunto não muda entre visitas. Faces iguais às do feltro nesta mão **não** obrigam troca. Detalhe visual das faces fica para o plan (mesmo idioma da mesa).
...
- **Overlay**: a colinha flutua no canto e **não** desloca feltro, assentos nem HUD. No 1280×720 a 100% cabe sem rolagem. Fora disso, encolher ou rolar interno — nunca cobrir a mesa. Ajuste fino de espaçamento da mesa, se o implement 007 precisar, é converge posterior da 001 — não altera o contrato CA-001 nem esta spec.
- **Teclado**: só **Ocultar** e **Colinha** entram no ciclo de Tab da colinha. As linhas são consulta visual, não controles de quiz. Sem armadilha de foco; foco após toggle fica no controle que restou; abertura sem autofoco na colinha. Só **Ocultar** dispensa o painel (não Escape, não clique fora). Ordem de Tab da página: HUD primeiro (CTAs e opções), colinha depois.
